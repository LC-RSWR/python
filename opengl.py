...
class OpenGLWidget(QGLWidget):
    ...
    def initializeGL(self):
        ...
        glClearColor(0.7, 0.9, 1.0, 1)  # 设置背景颜色为浅蓝色

    def load_model(self, stl_file_path):
        self.model = mesh.Mesh.from_file(stl_file_path)

    def paintGL(self):
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
        glLoadIdentity()
        glTranslatef(0, 0, -5)  # 移动视角

        if self.model is not None:
            glBegin(GL_TRIANGLES)
            for face in self.model.vectors:
                for vertex in face:
                    glVertex3fv(vertex)
            glEnd()
...
class MainWindow(QMainWindow):
    ...
    def load_files(self, index):
        ...
        self.opengl_widget.load_model(stl_file_path)
